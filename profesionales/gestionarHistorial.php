...
if ($idPaciente) {
    ...
    if ($resumenPaciente) {
        ...
        // Obtener el último total de puntos del paciente seleccionado
$queryTotalPuntos = "SELECT total 
FROM puntos 
WHERE idUsuario = ? 
ORDER BY fechaRegistro DESC, idPuntos DESC 
LIMIT 1";
$stmtPuntos = $conn->prepare($queryTotalPuntos);
$stmtPuntos->bind_param("i", $idUsuario);
$stmtPuntos->execute();
$resultPuntos = $stmtPuntos->get_result();
$totalPuntos = $resultPuntos->fetch_assoc()['total'] ?? 0; // Usar 0 si no hay puntos registrados
